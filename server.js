...
async function handleFormSubmission(req, res, formType) {
  try {
    ...
    await transporter.sendMail({
      ...
      subject: `Agarwal Website Lead: ${normalized.form_type}`,
      text: Object.entries(normalized)
        .map(([key, value]) => `${key}: ${value || '-'}`)
        .join('\n'),
      html: buildMailHtml('New Agarwal Website Lead', normalized)
    });

    sendJson(res, 200, {
      ok: true,
      message: 'Thanks! Your request has been sent to Agarwal Logistics Packers and Movers.'
    });
  } catch (error) {
    sendJson(res, 500, {
      ok: false,
...
server.listen(PORT, HOST, () => {
  console.log(`Agarwal site running at http://${HOST === '0.0.0.0' ? 'localhost' : HOST}:${PORT}`);
});
